...
def get_global() -> list[list|frd.SRC_Error, list|frd.SRC_Error]:
    ...
    frd.file_additional(file='../src-history/src-log.txt',
                        content=f'$ SRC start at {frd.ftime()} {frd.time_stamp()} >\n')  # 作为第一个运行的函数向log文件提示SRC运行
...
###### 全局设置
seed = frd.time_stamp()
random.seed(seed)  # 使用当前的时间戳作为随机算法的种子
print(f'当前时间戳（从1900年1月1日00时00分00秒开始到当前的秒数）为： {seed}')
print(f'为随机算法提供种子：{seed}')
del(seed)
